...
### MODEL HELP TO NAVIGATE USER
from sentence_transformers import SentenceTransformer , util
import numpy as np

sentence_sim_model = SentenceTransformer('sentence-transformers/all-mpnet-base-v2')

sentence_2_compare = ['I want go to virtual fitting room, and try on, not need to recommend anything' , 
                      'I want provide clothes, outfit recommendation ',
                      'Recommend base on my image']
...
def has_recommend_image(question):
    #special_context = ['recommend' , 'score']
    special_context = ['image']
...
async def upload_and_rating():
    ...
    file_image = file[0]
    element = [cl.Image(name="image1", display="inline", content=file_image.content)]
    
    
    msg = cl.Message(content=f"Processing `{file_image.name}`…", elements = element)
    await msg.send()
    ...
    msg_2 = cl.Message(content=f"`{file_image.name}` processed. Please while for the response ...!")
    await msg_2.send()
    ...
    image_dict = []
    for item in sources:
        if item.metadata['relevance_score']>0.1:
...
async def scoring_outfit () :
    ...
    file2 = await cl.AskFileMessagle(
        content="Please upload your image", accept = {'image/png': ['.png' ,'.jpg' ,'.jpeg']}
    ).send()
    
    dataBytesIO1 = io.BytesIO(file1[0].content)
    raw_image1 = Image.open(dataBytesIO1).convert('RGB')
    text1 = "a person is wearing "
    inputs1 = processor(raw_image1, text1, return_tensors="pt")
    out1 = caption_model.generate(**inputs1 )
    image_caption1 = processor.decode(out1[0], skip_special_tokens=True , max_length = 100)
    
    
    dataBytesIO2 = io.BytesIO(file2[0].content)
    raw_image2 = Image.open(dataBytesIO2).convert('RGB')
    text2 = "a person is wearing "
    inputs2 = processor(raw_image2, text2, return_tensors="pt")
    out2 = caption_model.generate(**inputs2 )
    image_caption2 = processor.decode(out2[0], skip_special_tokens=True , max_length = 100)
    
    input = "With the following outfit" + image_caption1.split(text1)[-1] + " and " + image_caption2.split (text2)[-1] + "what do you think and give me the recommendation for better outfit"
    chain = cl.user_session.get("chain") 
    cb = cl.AsyncLangchainCallbackHandler(
    stream_final_answer=True, answer_prefix_tokens=["FINAL", "ANSWER"])
    cb.answer_reached = True
...
async def show_image(answer , image_dict):  
    for item in image_dict:
        ...
        if item[4][-4:] == '.png':
            elements = []
            elements.append(cl.Image(name="image1", display="inline", path=item[4]))
            answer = f"Product name: {item[0]}\nProduct description: {item[1]}\nProduct color: {item[2]}\nProduct price: {item[3]}"
...
async def on_action(action):
    ...
    webbrowser.open_new_tab('https://7ab1-14-241-254-209.ngrok-free.app/?fbclid=IwAR3tbUMIlIG6dPcPKI6HYpcknrj3Iv0iiO7x84Yl5znLPx3oGF_0ZCxJAFw') 

async def show_button():
    actions = [
        cl.Action(name="enter_virtual_fittingroom", value="example_value", description="Click me!")
    ]
    await cl.Message(content="Of course, click this button and i will take you to the fitting room", actions=actions).send()
...
async def init():
    ...
    # Đọc dữ liệu từ tệp PDF dưới dạng bytes
    with open(pdf_path, 'rb') as file:
        pdf_data = file.read()

    # # Tạo một đối tượng BytesIO từ dữ liệu bytes của tệp PDF
    # pdf_stream = BytesIO(pdf_data)
    # file = files[0]
    msg = cl.Message(content="Initializing chatbot…")
    await msg.send()
    # # Read the PDF file
    pdf_stream = BytesIO(pdf_data)
    ...
    msg.content = "Chatbot processed. You can now ask questions!"
    await msg.update()
...
async def process_response(res:cl.Message):
    ...
    # new_question = res.content + ". NOT SHOWING PAT"
    # response = await chain.acall(new_question, callbacks=[cb])
    # print(f"response: {response}")
    # answer = response["result"] #quan trong dung de lay cau tra loi

    ##BATDAU EMBEDIING DE LAY KEt QUA :
    user_embedding = sentence_sim_model.encode (res.content)
    scores = []
    for sentence in sentence_2_compare :
        sentence_embedding = sentence_sim_model.encode (sentence)
    ...
    if np.max ( scores ) < 0.2 :
        ...
        dialog_option = np.argmax (np.array (scores)) 
    ...
    #         cl.user_session.set("chain", chain)
     ...
     ## 0 : go to virtual fitting room, 
    # #1 : recommend outfit with text,
    # 2 :  recommend outfit with image,
    # 3 : not related to fashion
    else:
        if dialog_option == 0 :
            print ("OPTION 0")
            run_sync (show_button())
            cl.user_session.set("chain", chain)
        elif dialog_option == 1:
            print ("OPTION 1")
            new_propmt = res.content
            cb.answer_reached = True
            response = await chain.acall(new_propmt, callbacks=[cb])
            ...
            for item in sources:
                ...
                if item.metadata['relevance_score']>0.3:
                    item.page_content = item.page_content.replace('\n','')
                    item_data = []
                    pattern = r"\[PRODUCT NAME\]: (.+?);"
                    match = re.search(pattern, item.page_content)
            ...
            if len(image_dict)>0:
                ...
                await cl.Message(content="I don't have any product about this, can you find another product?").send()
            ...
            print ("OPTION 3")
            new_prompt = 'User are talking about something that not related to the topic, ask user for focus on the main factor.'
            cb.answer_reached = True
            response = await chain.acall(new_prompt, callbacks=[cb])
            answer = response["result"]
            await cl.Message(content=answer).send()
            cl.user_session.set("chain", chain)
